...
#define USE_RING_BUFFER_AS_MODULE
...
#if defined(USE_RING_BUFFER_AS_MODULE)
...
avg_mag_t avg_mag = { 0, 0, 0 };

uint32_t calculate_magnitude(point3d_t* point)
{
    point->magnitude = (uint32_t)sqrt(((point->x * point->x) + (point->y * point->y) + (point->z * point->z)));
    return point->magnitude;
}

void* rb_avg_mag_callback(void* accumulated_value, void* data)
{
    avg_mag_t* ptr = (avg_mag_t* )accumulated_value;
    point3d_t* item = (point3d_t* )data;
    ptr->sum += item->magnitude;
    if (ptr->count == 124) {
        std::cout << "reached 125" << std::endl;
    }
    ptr->count++;
...
int test_ring_buffer_as_module(void)
{
    ...
    for (uint32_t i = 0; i < 1000; i ++) {
        ...
        rb_enqueue(rb_handle,&gyro_data);

        avg_mag.count = 0;
        avg_mag.sum = 0;
        rb_inject(rb_handle,&avg_mag,rb_avg_mag_callback);
